...
def sync_from_csv():
    ...
    for row in read_csv(CSV_DIR / "users.csv"):
        upsert_user(
            row.get("name"),
            row.get("email"),
            row.get("password"),
    ...
    for row in read_csv(CSV_DIR / "merchants.csv"):
        upsert_merchant(
    ...
    for merchant in merchants:
        csv_path = CSV_DIR / f"{merchant['business_name']}.csv"
        for row in read_csv(csv_path):
            if str(row.get("Dismissed") or "").lower() == "true":
                continue
            if row.get("VoucherTitle"):
                upsert_voucher(merchant["business_name"], row)

    # Convert old feedback rows into support tickets for admin visibility.
    for row in read_csv(CSV_DIR / "feedback.csv"):
...
def dashboard_stats():
    ...
    return {
        ...
        "today_redemptions": query_one("SELECT COUNT(*) AS c FROM voucher_redemptions WHERE substr(redeemed_at, 1, 10) = ?", (today,))["c"],
        "total_redemptions": query_one("SELECT COALESCE(SUM(redeemed_count), 0) AS c FROM vouchers")["c"],
