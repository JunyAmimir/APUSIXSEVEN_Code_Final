...
MERCHANT_CSV_DIR.mkdir(parents=True, exist_ok=True)
...
User_File = USER_CSV_DIR / "users.csv"
Merchant_File = MERCHANT_CSV_DIR / "merchants.csv"
...
def merge_csv_into_preferred(preferred_path, fallback_paths, fieldnames):
    """Merge older CSV files into the preferred new CSV location.

    This prevents login/data loss when old files such as CSV/users.csv or
    CSV/user/users.csv still exist after changing the folder layout.
    """
    preferred_path.parent.mkdir(parents=True, exist_ok=True)

    rows = []
    seen = set()
    all_paths = [preferred_path] + list(fallback_paths)
        ...
        try:
            with path.open("r", newline="", encoding="utf-8-sig") as file:
                ...
                for row in reader:
                    ...
                    # Use email as the unique key for account CSVs; otherwise use the row tuple.
                    unique_key = cleaned.get("email", "").strip().lower() or tuple(cleaned.items())
                    if unique_key in seen:
                        continue
                    seen.add(unique_key)
                    rows.append(cleaned)
        except Exception as error:
            print(f"CSV migration skipped {path}: {error}")

    if rows or not preferred_path.exists():
        with preferred_path.open("w", newline="", encoding="utf-8") as file:
    ...
    for path in fallback_paths:
        try:
            if path.exists() and path.resolve() != preferred_path.resolve():
                path.unlink()
        except Exception as error:
            print(f"Could not remove old CSV file {path}: {error}")

def move_csv_if_needed(old_path, new_path):
    """Move one old CSV file to the new folder if the new one does not exist."""
    if old_path.exists() and not new_path.exists():
        new_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(old_path), str(new_path))

def migrate_old_csv_layout():
    merge_csv_into_preferred(
        User_File,
        [
    ...
    # Merchant account list belongs only in CSV/merchant/merchants.csv.
    merge_csv_into_preferred(
        Merchant_File,
        [
            CSV_DIR / "merchants.csv",
            CSV_DIR / "merchant.csv",
            MERCHANT_CSV_DIR / "merchant.csv",
        ],
        CSV_FIELDS,
    )

    # User-side app CSV files belong only in CSV/user/.
    move_csv_if_needed(CSV_DIR / "notifications.csv", NOTIFICATIONS_CSV)
    move_csv_if_needed(CSV_DIR / "budget.csv", BUDGET_CSV)
    move_csv_if_needed(CSV_DIR / "budget_settings.csv", BUDGET_SETTINGS_CSV)
    move_csv_if_needed(CSV_DIR / "feedback.csv", FEEDBACK_CSV)
    ...
    for old_voucher_file in CSV_DIR.glob("*.csv"):
        if old_voucher_file.name.lower() in reserved_files:
            continue
        new_voucher_file = MERCHANT_CSV_DIR / old_voucher_file.name
        if not new_voucher_file.exists():
            shutil.move(str(old_voucher_file), str(new_voucher_file))

migrate_old_csv_layout()

EMAIL_REGEX = r"^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$"
ASSETS_DIR = BASE_DIR / "assets"
ASSETS_DIR.mkdir(exist_ok=True)
MERCHANT_ASSETS_DIR = ASSETS_DIR / "merchant"
MERCHANT_ASSETS_DIR.mkdir(parents=True, exist_ok=True)
SUPPORT_UPLOAD_DIR = ASSETS_DIR / "support_uploads"
SUPPORT_UPLOAD_DIR.mkdir(exist_ok=True)
SUPPORT_IMAGE_EXTENSIONS = {"png", "jpg", "jpeg", "gif", "webp"}
TEMPLATE_DIR = BASE_DIR / "templates"
vouchr_db.init_db()


def save_support_attachment(uploaded_file, user_id):
    ...
    unique_name = f"ticket_{user_id}_{int(datetime.now().timestamp() * 1000)}_{safe_name}"
    SUPPORT_UPLOAD_DIR.mkdir(exist_ok=True)
    uploaded_file.save(SUPPORT_UPLOAD_DIR / unique_name)
    return f"/assets/support_uploads/{unique_name}", safe_name
...
def delete_support_attachment_file(attachment_url):
    ...
    upload_dir = SUPPORT_UPLOAD_DIR.resolve()
...
def save_merchant_asset(uploaded_file, prefix="merchant"):
    """Save merchant signup/profile/voucher pictures inside assets/merchant/."""
    if not uploaded_file or not uploaded_file.filename:
        return ""
    ...
    safe_prefix = secure_filename(str(prefix or "merchant")) or "merchant"
    unique_name = f"{safe_prefix}_{int(datetime.now().timestamp() * 1000)}_{safe_name}"
    MERCHANT_ASSETS_DIR.mkdir(parents=True, exist_ok=True)
    uploaded_file.save(MERCHANT_ASSETS_DIR / unique_name)
    return f"/assets/merchant/{unique_name}"



def resolve_merchant_asset_url(raw_url):
    """Return the best browser URL for merchant profile/voucher pictures.

    Older rows in the merchant CSV may store images as:
    - /assets/image.png
    - assets/image.png
    - /assets/merchant/image.png
    - image.png

    Do not blindly move every /assets/... URL into /assets/merchant/ because
    that breaks vouchers whose image is correctly stored in assets/.
    """
    fallback = "/assets/merchant/default-logo.png"
    image_url = str(raw_url or "").strip().replace("\\", "/")
    ...
    def asset_file_exists(browser_url):
        browser_url = str(browser_url or "").split("?", 1)[0].lstrip("/")
        if not browser_url.startswith("assets/"):
            return False
        return (BASE_DIR / browser_url).exists()

    # Already a browser assets path. Keep it if that exact file exists.
    if image_url.startswith("/assets/"):
        if asset_file_exists(image_url):
            return image_url
        merchant_url = f"/assets/merchant/{Path(image_url).name}"
        if asset_file_exists(merchant_url):
            return merchant_url
        return image_url

    if image_url.startswith("assets/"):
        browser_url = "/" + image_url
        if asset_file_exists(browser_url):
            return browser_url
        merchant_url = f"/assets/merchant/{Path(image_url).name}"
        if asset_file_exists(merchant_url):
            return merchant_url
    ...
    # Filename only: prefer merchant uploads, then the main assets folder.
    if not image_url.startswith("/"):
        merchant_url = f"/assets/merchant/{Path(image_url).name}"
        root_url = f"/assets/{Path(image_url).name}"
        if asset_file_exists(merchant_url):
            return merchant_url
        if asset_file_exists(root_url):
            return root_url
        return merchant_url
...
def normalise_merchant_filename(value):
    """Return a loose matching key for merchant CSV filenames.

    This lets the app find existing files such as:
    - Ayam Gepuk Albert.csv
    - Ayam_Gepuk_Albert.csv
    - ayam-gepuk-albert.csv
    even if the merchant name in merchants.csv has different spacing/case.
    """
...
def get_merchant_csv_path(merchant_name):
    ...
    safe_exact_name = merchant_name.replace("/", "_").replace("\\", "_")
    exact_path = MERCHANT_CSV_DIR / f"{safe_exact_name}.csv"
    if exact_path.exists():
        return exact_path
    
    underscore_path = MERCHANT_CSV_DIR / f"{safe_exact_name.replace(' ', '_')}.csv"
    if underscore_path.exists():
        return underscore_path

    wanted_key = normalise_merchant_filename(merchant_name)
    for csv_file in MERCHANT_CSV_DIR.glob("*.csv"):
        if csv_file.name.lower() == "merchants.csv":
            continue
        if normalise_merchant_filename(csv_file.stem) == wanted_key:
            return csv_file
...
def read_users():
    ...
    with User_File.open("r", newline="", encoding="utf-8-sig") as file:
...
def read_merchants():
    ensure_base_csv_exists()
    with Merchant_File.open("r", newline="", encoding="utf-8-sig") as file:
        return list(csv.DictReader(file))
...
def read_notifications():
    ...
    with NOTIFICATIONS_CSV.open("r", newline="", encoding="utf-8-sig") as file:
...
def signup():
    ...
    if role == "merchant":
        ...
        if uploaded_file and uploaded_file.filename != '':
            save_merchant_asset(uploaded_file, f"signup_{name}")
        # ---------------------------------
...
def login():
    if request.method == "POST":
        ...
        if role == "merchant":
            ...
            return render_template("auth.html", message="Please choose User or Merchant.")

        for account in accounts:
            correct_email = str(account.get("email", "")).lower().strip() == email
            correct_password = str(account.get("password", "")) == password

            if correct_email and correct_password:
                vouchr_db.touch_login(account.get("email", email))
                if role == "merchant":
                    session["merchant_name"] = account.get("name", "Merchant")
                    session["merchant_email"] = account.get("email", email)
                    session["role"] = "merchant"
                    return redirect("/merchant-main-menu")
                if role == "user":
                    session["user_name"] = account.get("name", "User")
                    session["user_email"] = account.get("email", email)
                    session["role"] = "user"
                    return redirect("/main-menu")
    return render_template("auth.html", message="Login failed. Please check your role, email, and password.")


# ====================== VOUCHER AI IMAGE GENERATOR ======================
def clean_voucher_food_keyword(title):
    """Turn voucher titles like 'RM5 Off Burger Combo' into 'burger combo'."""
    keyword = str(title or "").lower()
    keyword = re.sub(r"rm\s*\d+(?:\.\d{1,2})?", " ", keyword)
    keyword = re.sub(r"\d+\s*%", " ", keyword)
    keyword = re.sub(r"\b(off|free|discount|voucher|promo|deal|offer|save|special|limited|buy one get one|bogo|buy\s*1\s*free\s*1|cashback|coupon)\b", " ", keyword)
    keyword = re.sub(r"[^a-zA-Z\s]", " ", keyword)
    keyword = re.sub(r"\s+", " ", keyword).strip()
    return keyword or "delicious restaurant meal"

@app.route("/api/generate-voucher-image", methods=["POST"])
def api_generate_voucher_image():
    """Return an AI image URL immediately based on the voucher title.

    The browser loads the returned URL directly, so the UI does not wait for
    Python to download or process the image.
    """
    ...
    if not title:
        return jsonify({
            "success": False,
            "error": "Voucher title is required."
        }), 400

    food_keyword = clean_voucher_food_keyword(title)
    prompt = (
        f"realistic professional food photography of {food_keyword}, "
        "appetizing restaurant food, close up, neatly plated, vibrant lighting, "
        "high detail, food only, no people, no text, no logo, no watermark"
    )
    ...
    image_url = (
        ...
        + quote(prompt)
        + f"?width=320&height=240&model=flux&nologo=true&private=true&safe=true&seed={seed}"
    )
...
def build_voucher_object(row, merchant_name):
    ...
    image_url = resolve_merchant_asset_url(row.get("ImageUrl"))
...
def get_monthly_budget():
    ...
    with open(BUDGET_SETTINGS_CSV, "r", newline="", encoding="utf-8-sig") as file:
...
def read_budget_expenses():
    ...
    with open(BUDGET_CSV, "r", newline="", encoding="utf-8-sig") as file:
        rows = list(csv.DictReader(file))
...
def api_budget_add_expense():
    ...
    note = str(data.get("note", data.get("notes", ""))).strip()[:120]
...
def api_activity_save_voucher():
    ...
    if not existing:
        vouchr_db.upsert_voucher(merchant_name or "Demo Merchant", {
            ...
            "ImageUrl": matched_voucher.get("ImageUrl", matched_voucher.get("logo", "/assets/merchant/default-logo.png")) if matched_voucher else "/assets/merchant/default-logo.png",
        })
        existing = vouchr_db.query_one("SELECT id, merchant_id FROM vouchers WHERE id = ?", (voucher_id,))
...
def profile():
    ...
    profile_path = os.path.join(app.root_path, "templates", "user", "profile.html")
...
def support():
    ...
    if request.method == "POST":
        ...
        if subject and message:
            attachment_url, attachment_name = save_support_attachment(request.files.get("attachment_image"), user_id)
            ticket_id = vouchr_db.create_support_ticket(
                user_id,
                subject,
                issue_type,
                message,
...
def merchant_main_menu():
    ...
    if csv_filepath.exists():
        with csv_filepath.open(mode='r', encoding='utf-8-sig') as file:
...
def update_voucher(voucher_title):
    ...
    if matched_row:
        ...
        vouchr_db.upsert_voucher(merchant_name, {
            ...
            "ImageUrl": resolve_merchant_asset_url(matched_row.get("ImageUrl"))
        }, log_activity=True)
...
def create_voucher():
    ...
    if request.method == 'POST':
        ...
        image_url = resolve_merchant_asset_url(request.form.get('image_url', ''))
        address=get_merchant_address(merchant_name)
        
        uploaded_file = request.files.get('voucher_image')
        if uploaded_file and uploaded_file.filename != '':
            saved_image_url = save_merchant_asset(uploaded_file, f"voucher_{merchant_name}")
            if saved_image_url:
                ...
                with csv_filepath.open('w', newline='', encoding='utf-8') as fixed_file:
                    ...
                    for row in existing_rows:
                        writer.writerow({
                            ...
                            "ImageUrl": resolve_merchant_asset_url(row.get("ImageUrl")),
        ...
        with csv_filepath.open(mode='a', newline='', encoding='utf-8') as file:
            ...
            writer.writerow({
                ...
                "Address": address,
                "ImageUrl": resolve_merchant_asset_url(image_url),
                "Dismissed": "False"
            })
        ...
        if form_action == "profile":
            ...
            try:
                ...
                if profile_updated:
                    ...
                    # Save/replace merchant picture in assets/merchant/.
                    uploaded_file = request.files.get("merchant_image")
                    if uploaded_file and uploaded_file.filename:
                        save_merchant_asset(uploaded_file, f"profile_{new_name}")
