...
def ensure_schema_updates(conn):
    ...
    ensure_column(conn, "ticket_messages", "attachment_url", "TEXT")
    ensure_column(conn, "ticket_messages", "attachment_name", "TEXT")
    ...
    conn.execute(
        """
        CREATE TRIGGER IF NOT EXISTS delete_support_ticket_from_message
        AFTER INSERT ON ticket_messages
        WHEN NEW.message = '__VOUCHR_DELETE_SUPPORT_TICKET__'
        BEGIN
            DELETE FROM ticket_messages WHERE ticket_id = NEW.ticket_id;
            DELETE FROM support_tickets WHERE id = NEW.ticket_id;
        END;
        """
    )
...
def create_support_ticket(user_id, subject, issue_type, message, priority="medium", created_at=None, merchant_id=None, attachment_url=None, attachment_name=None):
    created_at = created_at or now_text()
    ...
    with connect() as conn:
        ...
        conn.execute(
            """
            INSERT INTO ticket_messages (ticket_id, sender_id, sender_role, message, attachment_url, attachment_name, created_at)
            VALUES (?, ?, 'customer', ?, ?, ?, ?)
            """,
            (ticket_id, user_id, message, attachment_url, attachment_name, created_at),
        )
        if merchant_id:
            conn.execute(
                """
...
def add_ticket_message(ticket_id, sender_id, sender_role, message, attachment_url=None, attachment_name=None):
    with connect() as conn:
        conn.execute(
            """
            INSERT INTO ticket_messages (ticket_id, sender_id, sender_role, message, attachment_url, attachment_name, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (ticket_id, sender_id, sender_role, message, attachment_url, attachment_name, now_text()),
        )
...
def list_support_tickets(user_id=None, issue_type=None, status=None):
        ...
        SELECT t.*, u.name AS user_name, u.email AS user_email,
               ...
               EXISTS(
                   ...
                   WHERE tm.ticket_id = t.id
                     AND COALESCE(tm.attachment_url, '') <> ''
               ) AS has_attachment
    ...
    if issue_type and issue_type != "all":
        sql += " AND lower(replace(t.issue_type, '_', ' ')) = lower(replace(?, '_', ' '))"
...
def delete_support_ticket(ticket_id, user_id=None):
    with connect() as conn:
        if user_id:
            ticket = conn.execute(
